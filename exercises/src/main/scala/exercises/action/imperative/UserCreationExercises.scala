...
object UserCreationApp extends App {
  ...
  readSubscribeToMailingListRetry(Console.system, 3)
}
...
object UserCreationExercises {
  ...
  def readSubscribeToMailingList(): Boolean = {
    ...
    val subscriptionIn = StdIn.readLine()
    subscriptionIn match {
      case "Y" => true
      case "N" => false
      case _   => throw new IllegalArgumentException(s"Invalid argument: $subscriptionIn")
    }
  }
  ...
  def readSubscribeToMailingList(console: Console): Boolean = {
    ...
    parseSubscription(console.readLine())
  }

  def parseSubscription(yN: String): Boolean =
    yN match {
      case "Y" => true
      case "N" => false
      case _   => throw new IllegalArgumentException(s"Invalid argument: $yN")
    }
  ...
  def readDateOfBirth(console: Console): LocalDate = {
    ...
    val dateOfBirth = console.readLine()
  ...
  def readUser(console: Console, clock: Clock): User = {
    ...
    val subscription = readSubscribeToMailingListRetry(console, 3)
    val time         = clock.now()
    val user         = User(name, dateOfBirth, subscription, time)
